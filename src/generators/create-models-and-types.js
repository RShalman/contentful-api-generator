...
const { capitalize } = require("../utils/commons.js");
const { findById } = require("./create-entries.js");
...
function createTypesTS() {
  const typesModel = JSON.parse(fs.readFileSync(__dirname + typesModelPath));
  ...
  if (typesModel) {
    ...
    const contentTypes = `${typesModel.reduce((acc, typeObj) => {
      ...
      const type = typeTemplate(name, fields, typeObj);
      return acc + type;
    }, "")}`;
