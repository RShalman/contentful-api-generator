...
} from "./import-contentful-data.js";
import {
  createTypesModels,
  createTypesTS,
} from "../generators/create-models-and-types.js";
...
export async function generateContentfulApi(args: CAGOptions) {
  const options = CreateOptions(args);

  await fetchContentfulSpaceData(options);
  if (options.downloadAssets) moveAssetsToPublicFolder(options);
